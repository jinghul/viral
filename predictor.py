...
def main(record):
    ...
    for train, test in kf.split(concat_feature):
        ...
        print('text')
        x[test, 1] = text_class.fit(x[train], ground_truth[train]).transform(text_feature[test,:])
        print(x[test,1].shape)
        print('social')
        x[test, 2] = social_class.fit(x[train], ground_truth[train]).transform(social_feature[test,:])
